...
class ContentDeliveryViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        // Sets up the UIs.
        navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .action, target: self, action: #selector(ContentDeliveryViewController.showContentManagerActionOptions))
    ...
    private func updateUserInterface() {
        ...
        //availableCacheSizeLabel.text = "sdaf"
    ...
    private func loadMoreContents() {
        ...
        print("Loading........Loading .........More Contents")
        manager.listAvailableContents(withPrefix: prefix, marker: marker, completionHandler: {[weak self](contents: [AWSContent]?, nextMarker: String?, error: Error?) -> Void in
    ...
    private func downloadObjectsToFillCache() {
        manager.listRecentContents(withPrefix: prefix, completionHandler: {[weak self](result: Any?, error: Error?) -> Void in
            guard let strongSelf = self else { return }
            if let downloadResult: [AWSContent] = result as? [AWSContent] {
                for content: AWSContent in downloadResult {
                    if !content.isCached && !content.isDirectory {
                        strongSelf.downloadContent(content: content, pinOnCompletion: false)
                    }
                }
            }
            } )
    ...
    private func showActionOptionsForContent(rect: CGRect, content: AWSContent) {
        ...
        let openRemoteAction = UIAlertAction(title: "Open Remote", style: .default, handler: {[unowned self](action: UIAlertAction) -> Void in
            self.openRemoteContent(content: content)
            })
    ...
    private func downloadContent(content: AWSContent, pinOnCompletion: Bool) {
        content.download( with: .ifNewerExists, pinOnCompletion: pinOnCompletion, progressBlock: {[weak self](content: AWSContent?, progress: Progress?) -> Void in
            ...
            if strongSelf.contents!.contains( where: {$0 == content}) {
                ...
                guard let strongSelf = self else { return }
                if let downloadError: Error = error as NSError? {
                    print("Failed to download a content from a server.\(downloadError)")
                    strongSelf.showSimpleAlertWithTitle(title: "Error", message: "Failed to download a content from a server.", cancelButtonTitle: "OK")
                }
                strongSelf.updateUserInterface()
            })
    }
    ...
    private func openContent(content: AWSContent) {
        if content.isAudioVideo() { // Video and sound files
            ...
            do {
            try content.cachedData.write(to: movieURL as URL, options: .completeFileProtection)
            } catch let error as NSError {
                print(error.localizedDescription)
            }
            //content.cachedData.writeToURL(movieURL, atomically: true) as URL
            
            let controller = AVPlayerViewController()
            let player = AVPlayer.init(url: movieURL as URL)
            controller.player = player
            controller.player?.prepareForInterfaceBuilder()
            ...
            showSimpleAlertWithTitle(title: "Sorry!", message: "We can only open image, video, and sound files.", cancelButtonTitle: "OK")
        }
    }
    
    // MARK: - OpenRemoteContent
    private func openRemoteContent(content: AWSContent) {
        content.getRemoteFileURL(completionHandler: { (url: URL?, error: Error?) -> Void in
            ...
            if content.isAudioVideo() { // Open Audio and Video files natively in app.
                ...
                print("Open other file types like PDF in web browser.")
                let storyboard = UIStoryboard(name: "ContentDelivery", bundle: nil)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if content.isDirectory {
            ...
            showActionOptionsForContent(rect: rowRect, content: content)
        }
    }
}
...
class ContentDeliveryCell: UITableViewCell {
    ...
    var content: AWSContent! {
        didSet{
            var displayFilename: NSString = content.key as NSString
            if let prefix: String = self.prefix {
                print(prefix)
                displayFilename = displayFilename.substring(from: 1) as NSString
                //displayFilename = displayFilename.substringFromIndex(prefix.endIndex)
            }
            
            fileNameLabel.text = displayFilename as String
            ...
            /*
            if content.knownRemoteLastModifiedDate.compare(content.downloadedDate) == .orderedAscending {
